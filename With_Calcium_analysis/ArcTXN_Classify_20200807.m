...
if ~size(dir_pre_cells,1)==0  % Find the date in the categorized folder
    i=1;
    while ~isempty(cell2mat(strfind({dir_pre_cells.name},[num2str(i) ,'.tif']))) && i<=size(cell_list,1)
        i=i+1;
        start_here=i;
    end
else
...
set(gcf, 'Position',  [50, 10, Window_size*size(im_aligned,2)+200, 1350])
cl=find(cell_list(:,3)>10 & cell_list(:,3)<size(im_aligned{1},3)-10);
...
% hold on
handles.axes2 = axes('Units','pixels','Position',[(Window_size+10)*(i-1)+50 1170 170 170]);
imagesc(max_crop_im{i})
axis equal tight off
colormap('gray')

handles.axes3 = axes('Units','pixels','Position',[(Window_size+10)*(i-1)+50 50 Window_size 1150]);
imshow(mont{i},[min(min(mont{i})) 1.1*max(max(mont{i}))])
colormap('gray')
axis equal tight off
handles.axes4 = axes('Units','pixels','Position',[(Window_size+10)*(i-1)+270 1170 170 170]);
...
sw(1,i)=input(['( ' num2str(j) '/' num2str(size(Cells.list,1)) ' )th cell of ' num2str(i) '/' num2str(size(im_aligned,2)) ' th image. Is the cell has TXN site?\n']);
...
for jj=1:size(sw_del,2)
    ...
    if sw_del(1,jj)==1
        delete([Class_folder '\' sp_name '\' num2str(jj) '\Non_TXN\',num2str(j),'.tif'])
    end
    
    if sw_del(1,jj)==2
        delete([Class_folder '\' sp_name '\' num2str(jj) '\TXN\',num2str(j),'.tif'])
    end
    
    if sw_del(1,jj)==3
        delete([Class_folder '\' sp_name '\' num2str(jj) '\NA\',num2str(j),'.tif'])
    end
...
if sw(1,k)==1 
    imwrite(uint16(max(crop_im{k}(:,:,jj),[],3)),[Class_folder '\' sp_name '\' num2str(k) '\Non_TXN\',num2str(j),'.tif'],'Compression','none');
end
if sw(1,k)==2
   imwrite(uint16(max(crop_im{k}(:,:,jj),[],3)),[Class_folder '\' sp_name '\' num2str(k) '\TXN\',num2str(j),'.tif'],'Compression','none');
end
if sw(1,k)==3
   imwrite(uint16(max(crop_im{k}(:,:,jj),[],3)),[Class_folder '\' sp_name '\' num2str(k) '\NA\',num2str(j),'.tif'],'Compression','none');
end
end
