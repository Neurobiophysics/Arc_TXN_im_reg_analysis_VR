function [p_value datum L]=plot_inform(dat,post_ref,y_lim,day_sw)
%conds=[1 1 1;2 1 1;1 2 1;1 1 2;2 2 1;2 1 2;1 2 2;2 2 2];
conds=[1;2];
xtick={'A1 Arc^-','A1 Arc^+','-+-','--+','++-','+-+','-++','+++'};
%xtick={'--','+-','++','-+','++-','+-+','-++','+++'};
%xtick={'--','+-','++','-+','++-','+-+','-++','+++'};
...
for m=1:size(dat,2)
    ...
    %     for d1=1:3
    %         for c=1:size(P,1)
    %             P{c,d1}(find(round(P{c,d1}(:,3)*30)>size(dat{m}.run{d1},1)),:)=[];
    %             P{c,d1}(find(dat{m}.run{d1}(round(P{c,d1}(:,3)*30),1)==0),:)=[];
    %         end
    %     end
    %
    %     [number_peak cat]=cellfun(@size,P);
    [deconMean]=cellfun(@decon_sum,S);
    ...
    if day_sw
        %    lambda{m}=[sum(number_peak(:,1:2),2)/sum(t_list(1,1:2)) number_peak(:,3)/t_list(1,3) sum(number_peak,2)/sum(t_list)]; % lambda_A lambda_B lambda_all
...
M=cellfun(@mean,datum); S=cellfun(@std,datum)./sqrt(N);
