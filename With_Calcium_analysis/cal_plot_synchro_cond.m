function [corr_mat corr_mat_pooled group_div pc p_val]=cal_plot_synchro_cond(dat,groups,sprt,post_ref)
bin=1;
clear list corr_mat
for m=1:size(dat,2)
    if post_ref
    ...
    list{m}{g+1,1}=setdiff([1:size(dat{m}.Arc,1)],cell2mat(list{m}))';
    for d1=1:3
        ...
        %Peaks=cellfun(@spike2array, dat{m}.S(cell2mat(list{m})',3+d1),'UniformOutput',false); % Cells consist of Peak arrays
        Peaks=cellfun(@spike2binnedarray, dat{m}.S(cell2mat(list{m})',3+d1),'UniformOutput',false);
        pc_list=dat{m}.isPC(cell2mat(list{m})',d1);
        Cal_trace=dat{m}.S(cell2mat(list{m})',3+d1); 
